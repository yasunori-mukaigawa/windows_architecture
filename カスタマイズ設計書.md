...
- **困ったときの参考ソース:**
    ...
    - DI（自動登録）の規約：`各レイヤの AssemblyMarker + Registrar`
...
|プロジェクト | 役割|
|---|---|
|IdIdentifyApp | 実行アプリ（Composition Root / ShellWindow / 起動）|
|IdIdentifyApp.Ui.Common | UI基盤（MVI / Navigation / Effect処理）|
|IdIdentifyApp.Common | 共通部品|
|IdIdentifyApp.Applications | UseCase / Ports
IdIdentifyApp.Domain | Entity / ValueObject / DomainError|
|IdIdentifyApp.Infrastructure | Repository / Gateway / DB / Config|
|IdIdentifyApp.Tests | テスト|

#### 重要ルール

DI登録は 各レイヤごとに完結させる  
Appは「束ねるだけ」  
ViewModelLocatorは App側に依存させない設計が前提  
...
3. **Reducer:** Messageを受け取り、新しい「State（状態）」を生成する。副作用は一切禁止。📝
...
### ■ AbsViewModel の内部処理（実装追従）

本基盤の ViewModel は、Intent を即時実行するのではなく、
内部の **Channel（キュー）を用いて順次処理**する。

#### 処理の流れ

1. View から `PublishIntentAsync` が呼ばれる
2. Intent は内部 Channel に積まれる
3. ViewModel 基底クラスが順次 `HandleIntentAsync` に渡す

また、Effect も同様に Channel によって管理される。

#### この設計の目的

- Intent の順序保証
- 並行実行による状態不整合の防止
- View 側の責務を最小化（Intent送信のみ）
...
ViewModel -up-> Message : 生成
ViewModel --> Effect : 発行
ViewModel --> State : 保持
ViewModel -> Intent : 受け取る
Reducer -> Message : Reduceで処理
ViewModel -left-> Reducer : 利用
...
SamplePageViewModel --> SamplePageReducer
...
1.  **画面表示を更新するメインストリーム**
    `View ➔ Intent ➔ ViewModel ➔ UseCase ➔ Message ➔ Reducer ➔ State ➔ View`
2.  **一度だけ実行するサイドエフェクト**
...
{
    private readonly SampleReducer _reducer;

    public SampleViewModel(SampleReducer reducer)
        : base(SampleState.Initial)
    {
        _reducer = reducer;
    }
    ...
    {
        return _reducer.Reduce(current, message);
    }
}
```

**4. Reducer: 状態更新ロジックを分離して実装**
```csharp
public sealed class SampleReducer
{
    public SampleState Reduce(SampleState current, SampleMessage message)
    {
...
**5. View (code-behind): Intentを飛ばすだけ**
```csharp
...
> - **Reducer** の中で `await` を使うこと（Reduceは常に同期処理であるべきです） ❌
...
### 2. この作業で追加するもの
1画面につき、以下の7ファイルをセットで追加するのがこの基盤のルールです。
- `Ui/Views/NewPage.xaml`
- `Ui/Views/NewPage.xaml.cs`
- `Ui/ViewModels/NewPageViewModel.cs`
- `Reducers/NewPageReducer.cs`
- `UiStates/NewPageState.cs`
- `Intents/NewPageIntent.cs`
- `Messages/NewPageMessage.cs`
...
5. Reducer で State へ反映し、XAML にバインドする。

#### DbContext 自動登録（実装追従）

DbContext を継承したクラスは、
Infrastructure 層で自動スキャンされ、
IDbContextFactory<T> として登録される。

##### ファイル命名規約
- OperationLogDatabase → operationlog.db

##### 初期化
アプリ起動時に AppDbContextInitializer により
DB初期化処理が実行される。
...
IAppConfig を実装した型は、
Infrastructure の自動登録処理により
JsonConfigProvider<T> として DI 登録される。

##### 命名規約
- ファイル名：型名.ToLowerInvariant() + ".json"

例：
FeatureToggleSettings → featuretogglesettings.json

#### 開発フロー
1. IAppConfig 実装を追加
2. JSONファイルを配置
3. UseCaseで IConfigProvider<T> を利用
...
2. **iewModel**: `HandleIntentAsync` 内で入力値を検証。
3. **ViewModel**: 検証結果を **Message** 化して `Dispatch`。
4. **Reducer:** Message を受け取り、**State** を更新する。
5. **ViewModel**: Reducer を通じて更新された State を保持する。
6. **View(XAML)**: `IsEnabled` や `TextBlock.Text` に **State** をバインド。
...
// Reducer
public sealed class CheckPage1Reducer
{
    public CheckPage1State Reduce(CheckPage1State current, CheckPage1Message message)
    {
        return message switch
        {
...
### 6. まとめ
- 入力起点であっても基本の流れは同じです。「Intent → Message → Reducer → State」のサイクルで統一します。
...
2. ViewModel で遷移可否を判定。
3. 条件を満たさない場合は `ShowDialogEffect` 等で通知。
4. 遷移可能な場合は `NavigateToEffect(typeof(TargetPage))` を発行。
5. ShellEffectCoordinator が Effect を購読し、NavigationService へ委譲して遷移を実行する。
...
ViewModel が発行した Effect は、
ShellWindow が直接処理するのではなく、
**ShellEffectCoordinator が購読・解釈する。**
...
この分離により：
- View は UI描画に専念できる
- ShellWindow はコンテナ責務に集中できる
- Effect 解釈ロジックが一元化される
...
### 6. まとめ
- 遷移は Effect で要求し、実際の実行は Shell/NavigationService　→　ShellEffectCoordinator 側に任せるのがこの基盤の鉄則です。
...
複数の UseCase を利用する場合、
ViewModel に個別注入するのではなく、
**機能単位の UseCases クラスに集約して注入する。**
...
2. **自動登録規約（Assemblyスキャン）の適用:**
   ...
   - **Reducer**: *`Reducer`（Transient / AsSelf）
...
public static IServiceCollection AddApplicationLayer(this IServiceCollection services)
{
    services.Scan(scan => scan
        .FromAssemblyOf<Applications.AssemblyMarker>()
        .AddClasses(c => c.Where(t => t.Name.EndsWith("UseCase")))
        .AsSelf()
...
```csharp
services.Scan(scan => scan
    .FromAssemblyOf<Infrastructure.AssemblyMarker>()
...
```csharp
public sealed class CustomerModuleServiceRegister : IModuleServiceRegistrar
{
    public void Register(IServiceCollection services)
    {
...
>- ❌ **`AppAssemblyMarker` で全レイヤを一括スキャンすること**
>- ❌ **DI責務を1箇所に集中させること（変更に弱くなる）**
    ...
    : AbsViewModel<SamplePageState, SamplePageMessage, SamplePageIntent>
{
    ...
    private readonly SamplePageReducer _reducer;

    public SamplePageViewModel(
        GetItemsUseCase useCase,
        SamplePageReducer reducer)
    ...
    private async Task HandleLoadAsync(CancellationToken ct)
    {
        ...
        if (result.IsSuccess)
        {
            Dispatch(new LoadSucceeded(
                string.Join(", ", result.Value!.Select(x => x.Name))));
            return;
        }
    ...
    protected override SamplePageState Reduce(
        SamplePageState state,
        SamplePageMessage message)
    {
        return _reducer.Reduce(state, message);
    }
}
```
...
// Reducer
// ==============================
public sealed class SamplePageReducer
{
    public SamplePageState Reduce(SamplePageState state, SamplePageMessage message)
    {
        return message switch
        {
            LoadStarted =>
...
**なぜダメか**
> [!CAUTION]
>- Reducerは「現在の状態 + メッセージ = 新しい状態」を導き出すための**純粋関数**であるべきだからです。
>- 副作用が混ざると、状態の変化が予測不能になり、デバッグやテストが極めて困難になります。

**⭕正解**
```text
Reducer は「届いたメッセージを元に、新しい状態を作るだけ」に徹する
```
...
- [ ] 例外は UseCase で捕捉され、ViewModelでは扱っていない
...
| **状態変更** | Message + Reducer | 画面をどう見せるか（表示） |
...
- [ ] **Effect 受信側（ShellWindow等）が動いているか**
    ...
    - `NavigateToEffect` に対する処理分岐が存在するか？
- [ ] **遷移先の Page 指定が正しいか**
...
- [ ] **Message ➔ State 反映ができているか**
    ...
    - Reducer で、受け取ったデータを表示用の型（文字列やリスト）に正しく変換しているか？
...
3. **Dispatch の呼び忘れ**
4. **Reducer 内で return current;（更新なし）している**
